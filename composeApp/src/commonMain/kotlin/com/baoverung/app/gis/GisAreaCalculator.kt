...
object GisAreaCalculator {
    ...
    fun calculatePolygonArea(points: List<GpsPoint>): Double {
        ...
        for (i in points.indices) {
            ...
            area += ((p2.longitude - p1.longitude) * PI / 180.0) * (2 + sin(p1.latitude * PI / 180.0) + sin(p2.latitude * PI / 180.0))
        }
    ...
    fun calculateBearing(lat1: Double, lon1: Double, lat2: Double, lon2: Double): Double {
        ...
        val brng = atan2(y, x) * 180.0 / PI
        return (brng + 360) % 360
    }

    fun formatDistance(meters: Double, unit: String = "Auto"): String {
        return when {
            unit == "km" || (unit == "Auto" && meters >= 1000) -> "${(meters / 1000.0).toString().take(5)} km"
    ...
    fun formatArea(sqMeters: Double, unit: String = "Auto"): String {
        return when {
            unit == "ha" || (unit == "Auto" && sqMeters >= 10000) -> "${(sqMeters / 10000.0).toString().take(5)} ha"
